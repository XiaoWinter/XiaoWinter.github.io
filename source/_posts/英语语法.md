...
2. where does + 主语(小三)+动词原型+（宾语）+(时间状语)
...
2. where  + is/am/are  + 主语 + going to + 动词原型 + （宾语）+（将来的时间状语）？
...
2. whose + 名词 + am/is/are + 主语+ going to +及物动词原型 + （将来时间状语）？
...
2. who is going to +动词原型+（宾语）+将来的时间状语？
...
3. when+there be + (状语) when+被动句+（地点状语）
...
2. what + a/an + 形容词+可数名词单数+主语+及物动词
...
含有“主语从句和表语从句” 的复合句不需要考虑从句和复合句之间的时态问题，都是根据表达的意思决定用什么时态，只要符合逻辑就可以了。但是，“含有宾语从句”的复合句一定要考虑其中的时态问题。宾语从句的时态都是取决于主句（复合句）的时态。
主句（复合句）使用现在时态（包括一般现在时和现在完成时）时，宾语从句可以使用任何时态，可以和主句事态一致，也可以根据实际情况自行决定。
...
学习定语从句，首先了解一个概念：被定语从句修饰的名词或代词称为**先行词**。
